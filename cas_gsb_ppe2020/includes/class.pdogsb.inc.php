...
class PdoGsb
{
    ...
    /**
     * Récupère toute la liste des visiteurs dans la base de donnée
     * @return $lesVisiteurs
     */
    public function getListeDesVisiteurs()
    {
        ...
        $requetePrepare->execute();
        $lesVisiteurs= array(); 
        while($unVisiteur = $requetePrepare->fetch()){ // Boucle qui récupère tous les visiteurs de la liste
    ...
    public function getNomVisiteur($id)
    {
        $requetePrepare = PdoGsb::$monPdo->prepare(
            ...
            'visiteur.prenom AS prenom FROM visiteur WHERE visiteur.id = :unId');
    ...
    /**
     * Retourne les mois pour lesquels un visiteur a une fiche à valider
     *
     * \param String $idVisiteur ID du visiteur
     *
     * \return un tableau associatif de clé un mois -aaaamm- et de valeurs
    ...
    /**
     * \Fonction qui permet la clôture des fiches de frais du mois précédent.
  ...
  * @param type $libelle
  * @param type $dateFrais
  * @param type $montant
  * @param type $idFiche
  * @return string
  */   
 public function reporteFraisHorsForfait($idVisiteur, $libelle, $dateFrais,$montant, $idFiche): string
    {
    ...
    /**
     * Fonction qui supprime le frais hors forfait dont l'id est passé en argument
     *
    ...
    public function getLesFiches($etat)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
            ...
            'FROM fichefrais ' .
            'LEFT OUTER JOIN visiteur ON fichefrais.idVisiteur=visiteur.id ' .
 ...
 /**
 * Fonction de remboursement des fiches "MP"
 ...
 */
 ...
 $moisPrecedent = getMois($laDate);
 if (((int) date("j")) >= 30) {
 $requetePrepare = PdoGSB::$monPdo->prepare('UPDATE ficheFrais ' . "SET
idetat = 'RB', datemodif = now() " . "WHERE fichefrais.mois = :unMois AND idetat =
'VA'");
 $requetePrepare->bindParam(':unMois', $moisPrecedent, PDO::PARAM_STR);
 $requetePrepare->execute();
 }
 ...
 /**
 /**
 * Fonction de mise en paiement des fiches "VA"
